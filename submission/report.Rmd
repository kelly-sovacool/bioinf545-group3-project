...
#Draw Alpha Diversity Plot 
pdf(file=here::here("figures", "AlphaDiversityPlot.pdf"), width = 4, height= 4) AlphaDiversity(input, map)
\begin{wrapfigure}{R}{0.5\textwidth}
    \begin{center}
        \includegraphics[width=0.49\textwidth]{../figures/AlphaDiversityPlot.pdf}  
        \caption{Alpha Diversity Plot a box plot of the shannon index in groups: healthy (H) or diagnosed with adenoma (A) or cancer (C). Shannon index measures how evenly the microbes are distributed in a sample.
}
        \label{fig:Alpha_Diversity_Plot}  
    \end{center}
\end{wrapfigure}

#Draw Beta Diversity Plot 
pdf(file=here::here("figures", "BetaDiversityPlot.pdf"), width = 5, height= 4) BetaDiversity(input, map)
dev.off()

\begin{wrapfigure}{R}{0.5\textwidth}
    \begin{center}
        \includegraphics[width=0.49\textwidth]{../figures/gene_abundance_MDS.pdf}  
        \caption{Beta diversity shows the difference between microbial communities, the difference in taxonomic abundance profiles from different samples, healthy (H) or diagnosed with adenoma (A) or cancer (C).}
        \label{fig:Beta_Diversity_Plot}  
    \end{center}
\end{wrapfigure}
...
Differentially abundant genes that were found in both adenoma and cancer patient groups were selected and used to map to metabolic pathways using the KEGG mapper (see Table \ref{tab:genes_neg} and Table \ref{tab:genes_pos}).
...
DEgenes_pos$Number.of.Genes <-
  DEgenes_pos$Number.of.Genes %>% replace_na("")
DEgenes_pos <-
  DEgenes_pos %>% select(-KEGG.No.) %>% filter(KEGG.Pathway != "")
kable(
  DEgenes_pos,
  ...
  kable_styling(font_size = 8) %>%
  row_spec(c(1, 14, 17, 20, 24, 27), bold = TRUE)
```
